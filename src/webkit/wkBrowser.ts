...
export class WKBrowser extends BrowserBase {
  ...
  _onDownloadCreated(payload: Protocol.Playwright.downloadCreatedPayload) {
    ...
    if (!page)
      return;
    const download = new Download(page._page, this._downloadsPath, payload.uuid);
    this._downloads.set(payload.uuid, download);
  }

  _onDownloadFinished(payload: Protocol.Playwright.downloadFinishedPayload) {
    const download = this._downloads.get(payload.uuid);
    if (!download)
      return;
    download._reportFinished(payload.error);
    this._downloads.delete(payload.uuid);
  }
